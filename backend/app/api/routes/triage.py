...
from app.services.triage import build_triage, read_triage
...
def get_triage(tickers: str | None = None, db: Session | None = Depends(get_db)) -> TriageResponse:
    return read_triage(db, tickers)


@router.post("/run", response_model=TriageResponse)
def run_triage(tickers: str | None = None, db: Session | None = Depends(get_db)) -> TriageResponse:
